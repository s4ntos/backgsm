...
CPMS_MATCHER=re.compile(r'^\+CPMS: (\d+),(\d+),\d+,\d+,\d+,\d+.*?$')
...
class GsmModem(object):
    ...
    def boot(self, reboot=False, pin=0):
        """
	...
	if pin > 0:
            self.command("AT+CPIN=%s" % (pin), raise_errors=False ) # send pin

        self.command("AT+CMUX=0", raise_errors=False) # try to have Internet and SMS
    ...
    def _fetch_stored_messages(self):
        ...
        """
        lines = self._strip_ok(self.command('AT+CPMS="SM","SM","SM"'))
	if len(lines)>0:
            m=CPMS_MATCHER.match(lines[0])
	
	while len(lines)>0:
	    if m is None:
               raise(errors.GsmReadError())
            lines.pop(0)
	    num_found,buffer1 = m.groups()
	    self._log("Queued SMS: %s , buffer: %s" % (num_found, buffer1))
            if int(num_found) == 0:
                return  num_found
